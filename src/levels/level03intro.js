...
function getLevel03intro() {
    ...
    const levelTitle = level.newEntity()
	  .add("variables", { value: "Level 3: Unfair fight"})
	  .getUuid();
    ...
    const levelManual = level.newEntity()
	  .add("variables", { value: "Let's check if you can win against two opponents.\n\n\
Whoever scores 7 points first wins.\n\n\
\n\nGood luck."})
	  .getUuid();

    const gameSubTitleText = level.newEntity()
	  .add("texts", { size: 20, variable: levelManual, color: 0xffffff, angle: 0, opacity: 0.6})
	  .add("positions", { x: Const.WORLD_WIDTH/2, y: 40 + 170, angle: 0})
	  .getUuid();
...
export default getLevel03intro;
