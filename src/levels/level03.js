...
function getLevel03() {
    ...
    const pointsNeededByPlayer = level.newEntity().add("variables",  { value: 7}).getUuid();
    const pointsNeededByEnemy = level.newEntity().add("variables",  { value: 7}).getUuid();
    ...
    const winAndDelay = level.newEntity()
	  .add("logicalAnds", [ playerHas7points, endTimePassed ])
	  .add("variables",  { value: null})
	  .getUuid();

    const loseAndDelay = level.newEntity()
	  .add("logicalAnds", [ enemyHas7Points, endTimePassed ])
	  .add("variables",  { value: null})
	  .getUuid();
    ...
    level.newEntity()
	.add("positions",     { x: Const.PADDLE_1_POSITION.X, y: Const.PADDLE_1_POSITION.Y - 10, angle: Math.PI})
	...
	.add("movements",     { minSpeed: 0, speed: 0, maxSpeed: Const.PADDLE_MAX_SPEED * 0.75, angle: 0, randomAngle: 0, enabled: ballCanMove})
    ...
    level.newEntity()
	.add("positions",     { x: Const.PADDLE_1_POSITION.X + 20, y: Const.PADDLE_1_POSITION.Y + 35, angle: Math.PI})
	.add("shapes",        { width: Const.PADDLE_WIDTH-20, height: Const.PADDLE_HEIGHT, angle: 0})
	.add("bodies",        { })
	.add("stopping",      { })
	.add("collisions",    [ ])
	.add("sprites",       { width: Const.PADDLE_WIDTH-20, height: Const.PADDLE_HEIGHT, angle: 0, image: LOLPIXELS, color: 0xff9999, opacity: 1.0})
	.add("movements",     { minSpeed: 0, speed: 0, maxSpeed: Const.PADDLE_MAX_SPEED * 0.4, angle: 0, randomAngle: 0, enabled: ballCanMove})
	.add("frictions",     { value: Const.PADDLE_FRICTION * 0.4})
	.add("springPivots",  { power: 0.09})
	.add("pivotLimiters", { minAngle: -0.20, maxAngle: 0.20})
	.add("accelerators",  { leftAccelerator:  {angle: Math.PI, acceleration: Const.PADDLE_ACCELERATION * 0.8},
				rightAccelerator: {angle: 0, acceleration: Const.PADDLE_ACCELERATION * 0.8} })
	.add("rotators",      { rightRotator: {speed: 0.3, direction: 1},
				leftRotator:  {speed: 0.3, direction: -1} })
	.add("orders",        { playerPaddleLeft: false,
				playerPaddleRight: false,
